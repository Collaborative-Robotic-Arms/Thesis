...
The Pinhole model serves as the ideal geometric basis for most visual servoing control laws. As illustrated in Figure~\ref{fig:pinhole_model}, a 3D point $P = [X, Y, Z]^T$ in the camera frame $\{C\}$ is projected onto the image plane at coordinates $p = [x, y]^T$ via the focal length $\lambda$\cite{Hutchinson1996}:
\begin{equation}
