...
The complete visual servoing pipeline integrates perception, transformation logic, and control as illustrated in Figure~\ref{fig:vs_architecture}, showing data flow from global perception through local feedback control. Two deep learning models extract object type and grasp point; these feed the homography calculation and target feature generation pipeline. The system operates in three primary stages:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{Figures/chapter3/VISP/VisualServoDiagram.png}
    \caption{Complete visual servoing system architecture.}
...
\begin{enumerate}
    ...
    \item \textbf{Morphological Processing:} Connected-component analysis identifies distinct blobs. Opening (erosion followed by dilation) removes noise and spurious small regions.
...
\begin{enumerate}
    ...
    \item \textbf{Rotation Offset:} The brick's orientation in the gripper varies depending on the grasp configuration. The system computes an adaptive \textbf{rotation offset} based on the shift vector from the grasp shifting module:
    \begin{equation}
        \text{offset}_{\text{rot}} = \begin{cases}
            \pi/2 & \text{if } |\Delta x| > |\Delta y| \text{ and } \Delta x > 0 \\
            -\pi/2 & \text{if } |\Delta x| > |\Delta y| \text{ and } \Delta x < 0 \\
            \pi & \text{if } |\Delta y| > |\Delta x| \text{ and } \Delta y > 0 \\
            0 & \text{if } |\Delta y| > |\Delta x| \text{ and } \Delta y < 0
        \end{cases}
    \end{equation}
    
    This ensures the brick's "graspable face" is always perpendicular to the camera's optical axis.
    
    \item \textbf{3D Rotation:} Apply the computed rotation to align the brick correctly:
    \begin{equation}
        \text{corners}_{rotated} = R(\text{offset}_{\text{rot}}) \cdot \text{corners}_{3D}^T
    \end{equation}
    
    \item \textbf{Perspective Projection:} Project the rotated 3D corners onto the image plane using the camera intrinsic matrix $K$:
    \begin{equation}
        p_{pixel} = K \cdot \frac{\text{corners}_{rotated}}{Z^*}
    \end{equation}
