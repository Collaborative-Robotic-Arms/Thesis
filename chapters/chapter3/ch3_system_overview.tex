...
\begin{itemize}
    ...
    \item \textbf{Sequential Execution Model}: The current implementation operates sequentially—while one arm (ABB or AR4) executes a motion, the other arm remains idle.
...
\begin{itemize}
    ...
    IK solvers are critical for:
    \begin{itemize}
        \item Converting Cartesian end-effector poses (desired pick/place positions) to joint configurations
        \item Finding multiple solution branches to select collision-free configurations
        \item Incorporating constraints (joint limits, reachability limits)
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Object Detection (YOLOv8 Segmentation)}: Uses the state-of-the-art YOLOv8 segmentation model to:
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{6-DOF Pose Prediction}: Estimates gripper orientation and position relative to the brick
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Domain adaptation techniques to transfer synthetic knowledge to real scenarios
    \end{itemize}
    
    The output is a ranked list of candidate grasps with confidence scores, allowing the Planning Module to select collision-free grasps.
    ...
    \item \textbf{Planar Homography (Optional Disambiguation)}: For challenging scenarios (occluded bricks, cluttered scenes), planar homography estimates 3D brick locations from 2D image coordinates assuming bricks lie on a known plane (the work surface):
    \begin{itemize}
        \item Homography matrix $H$ is computed from known point correspondences
        \item 2D image points are lifted to 3D using the plane constraint
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item If grasp learning fails: Use geometric grasp estimators (surface normal analysis)
        \item If YOLOv8 fails (rare cases): Switch to simpler detection methods or manual intervention
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Motor control boards managing PWM signals and current feedback
\end{itemize}
...
\begin{itemize}
    ...
    \item Provides depth for 3D reconstruction 
\end{itemize}

\textbf{ESP32 Wrist Camera}: Custom lightweight camera module mounted on AR4 wrist:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{User Input}: Operator specifies a brick picking and placing task in the GUI
    ...
    \item \textbf{Control}: Control Module executes trajectories with real-time feedback from encoders
...
\begin{enumerate}
    \item \textbf{Sensor Readings}: Hardware layer generates continuous sensor data (joint angles, camera frames)
...
\begin{itemize}
    ...
    \item \textbf{Control $\leftrightarrow$ Perception}: Control Module may request precision updates from Perception during delicate operations
...
\begin{itemize}
    ...
    \item \textbf{Emergency Stop}: E-stop triggers immediate motion cessation at HAL layer
\end{itemize}

\subsubsection{Scalability}
The ROS 2 architecture scales to:
\begin{itemize}
    \item Multiple robots (demonstrated with ABB + AR4)
    \item Additional sensors and actuators
    \item Distributed computation across multiple machines
    \item Future expansion to larger swarms of robots
\end{itemize}
