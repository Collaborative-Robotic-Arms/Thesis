...
The behavior of the visual servoing controller was evaluated under constrained rotational conditions. In this simulation scenario, the controller was configured to restrict degrees of freedom in the camera's roll and pitch axes, permitting movement only in translation ($v_x, v_y, v_z$) and yaw rotation ($\omega_z$). The results are presented in Figure~\ref{fig:sim_results_constrained}.
\begin{figure}[H]
    \centering
    % First Subfigure
    ...
    \caption{\textbf{Simulation Results (Constrained Motion):} Performance of the IBVS controller configured with rotational constraints. Top: Commanded camera angular velocities. Bottom: Commanded linear velocities. Note that roll ($\omega_x$) and pitch ($\omega_y$) rotations are explicitly restricted to zero throughout the task.}
