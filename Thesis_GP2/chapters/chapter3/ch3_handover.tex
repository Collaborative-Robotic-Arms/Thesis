...
The handover sequence is governed by a strict "Give-and-Take" handshake protocol within the Supervisor node. The explicit interaction timeline, transactional boundaries, and multi-node callback hooks driving this state machine are structured chronologically in Figure~\ref{fig:handover_sequence}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/chapter3/ch3_handover/Give_and_Take_Sequence_Diagram.png}
    \caption{UML sequence diagram tracking data exchanges and coordination during the collaborative handover.}
    \label{fig:handover_sequence}
\end{figure}

The discrete steps executed within this state boundary progress as follows:
\begin{enumerate}
...
A unique collaborative challenge arises when handing over asymmetric parts, such as "Z-shape" bricks. The system must ensure that the receiving gripper grasps a viable flat surface without colliding with the giver's gripper. The Supervisor implements dynamic presentation logic to solve this. 

By calculating the spatial differential between the AI-generated grasp point (the center of mass/optimal grasp) and the YOLO-detected geometric center of the brick, the system determines the dominant offset axis. Based on this differential, the giver arm automatically applies a $90^\circ$ or $180^\circ$ yaw rotation to its presentation angle. This rotational logic guarantees the receiver arm is presented with a clear, unobstructed parallel surface for the final take.