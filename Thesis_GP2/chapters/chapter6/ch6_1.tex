...
\begin{figure}[H]
    ...
    \caption{High-level ROS 2 node topology. Solid arrows are topic subscriptions, dashed arrows are ROS 2 actions, and dotted arrows are services. The red link from the zone detector preempts ongoing motion goals on the supervisor.}
...
\begin{figure}[H]
    ...
    \caption{MoveIt 2 motion-planning request lifecycle. A failed plan is retried up to the configured attempt count before the goal is returned as FAILURE to the supervisor.}
...
\begin{figure}[H]
    ...
    \caption{Hardware driver data paths from physical devices to the ROS 2 application layer. All three drivers publish standard ROS 2 message types, decoupling the hardware specifics from the planning and supervisory logic.}
