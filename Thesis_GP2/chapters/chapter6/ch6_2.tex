...
top of the ZeroTier virtual overlay described in Section~6.1.
...
\caption{Layered communication stack used by the dual-laptop deployment.
         ZeroTier provides cross-internet connectivity;
         Fast~DDS and Syncthing each handle a distinct class of payload.}
...
\end{tikzpicture}
\caption{Fast~DDS unicast discovery and steady-state topic flow between the two laptops.
         SPDP and SEDP exchanges replace the usual multicast announces.}
...
\end{tikzpicture}
\caption{Syncthing program-storage transfer lifecycle.  Each step executes automatically
         whenever the control laptop regenerates the YAML plan.}
...
\caption{Complete two-channel communication architecture.
         The Fast~DDS channel carries live telemetry from the station to the control laptop;
         the Syncthing channel carries the planned execution program in the reverse direction.}
