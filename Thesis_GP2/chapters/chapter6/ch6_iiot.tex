...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.9\textwidth]{images/data_exchange_flow.png}
...
\begin{figure}[htbp]
    ...
    % \includegraphics[width=0.9\textwidth]{images/data_exchange_flow.png}
    \rule{0.9\textwidth}{6cm} % Placeholder box
    \caption{Data-exchange flow between the control and station laptops. The station streams brick detections, joint states, and status flags to the control laptop over ROS2/Fast~DDS, while the control laptop returns the planned program storage (\texttt{YAML}) to the station over Syncthing; both channels run on the shared ZeroTier overlay.}
